...
def generateMatrix(args):
    ...
    for label in labels:
        ...
        args.evalLabels.append(label.categoryId)
    args.evalLabels = np.unique(args.evalLabels)
...
def printConfMatrix(confMatrix, args):
    ...
    for label in args.evalLabels:
        ...
        print("\b{text:^{width}} |".format(width=args.printRow, text=getCatName(label)[0]), end=' ')
...
def evaluateImgLists(predictionImgList, groundTruthImgList, args):
    ...
    for i in range(len(predictionImgList)):
        ...
        pixels, score =evaluatePair(predictionImgFileName, groundTruthImgFileName, confMatrix, instStats, perImageStats,
                                 args)
        nbPixels += pixels
        iou_score += score
    ...
    print(f'IoU score is {iou_score/len(predictionImgList)}')
    # sanity check
    if confMatrix.sum() != nbPixels:
        printError(
            'Number of analyzed pixels and entries in confusion matrix disagree: contMatrix {}, pixels {}'.format(
